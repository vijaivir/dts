...
const TradingPanel = (props) => {
    ...
    useEffect(() => {
        ...
        const price = getQuoteService(props.username, symbol)
        setStockPrice(price)
        setPoint(price)
    }, [symbol]);
    ...
    const renderSubmitButtons = () => {
        return <div>{(point === stockPrice) ? <button onClick={() => submitOperation()}>{operation}</button> : <button>Set {operation}</button>}</div>
    }

    const renderCommitButtons = () => {
        return <div><button onClick={() => setSubmitted(false)}>Cancel</button><button onClick={() => commitOperation()}>Commit {operation}</button></div>
    }
    ...
    return (
        <>
            <div>
                ...
                <div>
                    {submitted ? renderCommitButtons() : renderSubmitButtons()}
                </div>
            </div>
        </>
    );
